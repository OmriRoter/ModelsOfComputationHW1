...
*/
	...
	/**
	 * Determines whether the DFSM accepts or rejects the given input string.
	 *
	 * @param input the string to compute
	 * @return true if the DFSM accepts the input string, false otherwise
	 * @throws IllegalArgumentException if the input string contains symbols that are not in the DFSM's alphabet
	 */
	public boolean compute(String input) {
		// Check if the DFSM is defined properly (initial state exists and there are defined states)
		if (initialState == null || states.isEmpty()) {
			return false; // Reject if no initial state or no states defined
		}
		...
		for (int i = 0; i < input.length(); i++) {
			...
			// Check if the current symbol is in the DFSM's alphabet
			if (!alphabet.contains(symbol)) {
				throw new IllegalArgumentException("Input string contains invalid symbol: " + symbol);
			}

			// Transition to the next state based on the current state and symbol
			currentState = transitions.applyTo(currentState, symbol);
			// Check if the transition was valid
			if (currentState == null) {
				return false; // Reject if no valid transition exists
			}
		}

		// After processing the entire input string, check if the final state is an accepting state
